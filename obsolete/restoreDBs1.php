...
if($_POST)
{
   if(!isset($_POST['mName']) || strlen($_POST['mName'])<1)
	...
	if($_FILES['file-0']['error'])
    {
        ...
        die(upload_errors($_FILES['file-0']['error']));
    }

    $FileName           = strtolower($_FILES['file-0']['name']); //uploaded file name
    $FileTitle          = mysql_real_escape_string($_POST['mName']); // file title
    $FileExt            = substr($FileName, strrpos($FileName, '.')); //file extension
    $FileType           = $_FILES['file-0']['type']; //file type
    $FileSize           = $_FILES['file-0']["size"]; //file size
 ...
 /*   $NewFileName = preg_replace(array('/s/', '/.[.]+/', '/^[\w\_.-]/'), array('_', '.', ''), strtolower($FileTitle));  */
    ...
    if(move_uploaded_file($_FILES['file-0']["tmp_name"], $sqlFile ))
    {
